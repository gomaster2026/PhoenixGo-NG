...
void process_bn_var(container& weights, const float epsilon = 1e-5f) {
    for (auto&& w : weights) {
...
std::pair<int, int> Network::load_v3_network(std::istream& wtfile) {
    ...
    // Input conv: 1 unit (6 lines) — if PG input layer has no BN, identity BN is supplied
    ...
    while (std::getline(wtfile, line)) {
        ...
        if (linecount < plain_conv_wts) {
            ...
            if (line_in_unit == 0) {
                ...
                process_bn_var(weights, 1e-3f);
                m_fwd_weights->m_batchnorm_stddevs.emplace_back(weights);
            }
        } else if (linecount < head_start) {
            // Trunk BN (4 lines: gamma, beta, mean, var)
            ...
            switch (linecount - head_start) {
                ...
                case 9: m_fwd_weights->m_conv_val_b = std::move(weights); break;
                case 10:
                    std::copy(cbegin(weights), cend(weights),
                              begin(m_bn_val_g1));
                    break;
    ...
    process_bn_var(m_bn_pol_w2, 1e-3f);
...
void Network::initialize(const int playouts, const std::string& weightsfile) {
    ...
    if (m_format_version == 3) {
        ...
        //
        // Input: x_0 = conv_input(input) + bias_input  (no BN, no ReLU)
        //
        ...
        //   input_conv.bias      -> BN_res_0_0.mean
        //   bias_res_n_0         -> BN_res_n_1.mean
        //   bias_res_n_1         -> BN_res_{n+1}_0.mean  (or trunk_BN for last)
        //
        // Indexing (m_batchnorm_means[0] is input identity BN, unused in pre-act):
        ...
        if (bias_size == 0) {
            ...
            // input_conv.bias -> BN_res_0_0.mean (index 1)
            for (size_t j = 0; j < channels_per_layer; j++) {
                m_fwd_weights->m_batchnorm_means[1][j] -=
