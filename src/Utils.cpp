...
bool Utils::input_pending() {
    ...
    static HANDLE inh = INVALID_HANDLE_VALUE;
    DWORD dw;

    if (!init) {
        init = 1;
        inh = GetStdHandle(STD_INPUT_HANDLE);
        if (inh == INVALID_HANDLE_VALUE || inh == nullptr) {
            // No valid stdin handle; pretend input is pending so loops exit.
            return true;
        }
    ...
    if (pipe) {
        if (!PeekNamedPipe(inh, nullptr, 0, nullptr, &dw, nullptr)) {
            // Pipe broken or not a named pipe; treat as input available.
            return true;
        }
        return dw != 0;
    } else {
        if (!GetNumberOfConsoleInputEvents(inh, &dw)) {
            // Console input handle gone; treat as input available.
            return true;
        }
        return dw > 1;
    }
