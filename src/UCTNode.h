...
public:
    ...
    // search tree.
    // PhoenixGo uses virtual_loss=1.0 (see etc/mcts_cpu.conf)
    ...
    // PhoenixGo: virtual_loss must enter PUCT formula (virtual_loss_mode=0)
    // See PhoenixGo mcts_engine.cc:469-501 SelectChild
    int get_virtual_loss() const { return m_virtual_loss.load(); }
