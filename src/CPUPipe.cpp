...
void CPUPipe::forward(const std::vector<float>& input,
    ...
    // PhoenixGo pre-activation: input conv has NO BN/ReLU.
    // Input BN (index 0, identity) is stored but unused — do not apply.
    // Input conv bias is accumulated and folded into all BN_res_n_0.mean
    // and trunk_BN.mean (see Network.cpp accumulated bias folding).
