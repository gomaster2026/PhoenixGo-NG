...
// PhoenixGo time management (see PhoenixGo etc/mcts_cpu.conf)
bool cfg_early_stop_enable;
int cfg_early_stop_check_every_ms;
float cfg_early_stop_sims_factor;
int cfg_early_stop_sims_threshold;
bool cfg_unstable_overtime_enable;
float cfg_unstable_overtime_time_factor;
bool cfg_behind_overtime_enable;
float cfg_behind_overtime_act_threshold;
float cfg_behind_overtime_time_factor;
bool cfg_time_control_enable;
...
void GTP::setup_default_parameters() {
    ...
    // PhoenixGo: keep AUTO for TimeControl's base time allocation (board_div=9),
    // but have_alternate_moves() is modified to always return true (no pruning).
    // PG has no prune_noncontenders / have_alternate_moves concept.
...
#endif
#endif
    // PhoenixGo: c_puct=2.5, PUCT uses sqrt(N) not sqrt(N*log(...))
    cfg_puct = 2.5f;
    cfg_logpuct = 0.015f;
    cfg_logconst = 1.7f;
    cfg_softmax_temp = 1.0f;
    // PhoenixGo: FPU uses default_act=0 (no reduction), set to 0 to match
    cfg_fpu_reduction = 0.0f;
    // PhoenixGo: v_resign=-0.9 (Q value) => winrate < 5%
    // -1 = use default 10%; we set 5 to match PhoenixGo
    cfg_resignpct = 5;
    cfg_noise = false;
    cfg_fpu_root_reduction = cfg_fpu_reduction;  // PhoenixGo: same as non-root (0.0)
    cfg_ci_alpha = 1e-5f;
    // PhoenixGo: PG has no LCB concept (see PhoenixGo mcts_engine.cc GetBestMove)
    // PG sorts by visit_count only. Set ratio > 1.0 so NodeComp's LCB condition
    // (visits > lcb_min_visits = ratio * max_visits) never triggers, making
    // sort_children fall back to pure visit-count ordering (PG behavior).
    cfg_lcb_min_visit_ratio = 2.0f;
    // PhoenixGo time management defaults (see PhoenixGo etc/mcts_cpu.conf)
    // early_stop: enable=1, check_every_ms=100, sims_factor=1.0, sims_threshold=2000
    cfg_early_stop_enable = true;
    cfg_early_stop_check_every_ms = 100;
    cfg_early_stop_sims_factor = 1.0f;
    cfg_early_stop_sims_threshold = 2000;
    // unstable_overtime: enable=1, time_factor=0.3
    cfg_unstable_overtime_enable = true;
    cfg_unstable_overtime_time_factor = 0.3f;
    // behind_overtime: enable=1, act_threshold=0.0, time_factor=0.3
    cfg_behind_overtime_enable = true;
    ...
    // score-based Pass override. dumb_pass suppression is handled in
    // create_children (policy capped to 1e-5 when passing loses).
    cfg_dumbpass = true;
