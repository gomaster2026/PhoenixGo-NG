...
bool UCTSearch::should_resign(const passflag_t passflag, const float besteval) {
    ...
    // PhoenixGo: no move_threshold restriction (see PhoenixGo mcts_engine.cc)
    // PG allows resign at any move number if v_resign threshold is met.
    // LZ original: blocked resign for first num_intersections/4 moves (~90 moves).
    // Removed to match PG resign behavior.
    ...
    // PhoenixGo: resign_mode=0 (see PhoenixGo mcts_engine.cc:1000)
    // v_resign = max(root_action, mean_action[choice])
    // Both root and best move must be below threshold to resign.
    const auto root_eval = m_root->get_visits() > 0
        ? m_root->get_raw_eval(color) : besteval;
    if (std::max(root_eval, besteval) > resign_threshold) {
        // eval > cfg_resign
        return false;
    }
...
bool UCTSearch::have_alternate_moves(const int elapsed_centis,
                                     const int time_for_move) {
    // PhoenixGo: PG has no prune_noncontenders / have_alternate_moves concept.
    // (see PhoenixGo mcts_engine.cc Search() - no candidate pruning)
    // Always return true to let search continue until time/visit limit
    // or PhoenixGo early_stop triggers.
    // LZ original: prunes non-contending root children to save time,
    // which PG does not do.
    (void)elapsed_centis;
    (void)time_for_move;
    return true;
}
...
// PhoenixGo: CheckEarlyStop (see PhoenixGo mcts_engine.cc:583-616)
// Stop early if best move's visit lead > estimated remaining sims.
bool UCTSearch::check_early_stop(const int time_for_move_centis,
                                 const int elapsed_centis) {
    if (!cfg_early_stop_enable
        || m_playouts.load() < cfg_early_stop_sims_threshold) {
        return false;
    }
    // Find top 2 visit counts among root children.
    int max_visit[2] = {0, 0};
    for (const auto& node : m_root->get_children()) {
        if (!node->valid()) continue;
        const auto vc = node->get_visits();
        if (vc > max_visit[1]) {
            if (vc > max_visit[0]) {
    ...
    if (elapsed_centis <= 0) return false;
    // PG: remain_sims = simulations * (timeout - elapsed) / elapsed
    auto remain_sims = static_cast<float>(m_playouts.load())
                       * (time_for_move_centis - elapsed_centis)
                       / elapsed_centis;
    if (cfg_early_stop_sims_factor > 0.0f) {
        remain_sims *= cfg_early_stop_sims_factor;
    }
    if (max_visit[0] - max_visit[1] > remain_sims) {
        myprintf("PG early stop: top1=%d top2=%d diff=%d remain_sims=%.0f\n",
                 max_visit[0], max_visit[1],
                 max_visit[0] - max_visit[1], remain_sims);
        return true;
    }
    return false;
}

// PhoenixGo: CheckUnstable (see PhoenixGo mcts_engine.cc:618-644)
// True if most-visited child != highest-Q child.
bool UCTSearch::check_unstable() {
    if (!cfg_unstable_overtime_enable) return false;
    const auto color = m_rootstate.board.get_to_move();
    auto n_best = -1;
    auto n_best_visits = 0;
    auto q_best = -1;
    auto q_best_eval = -1.0f;
    auto idx = 0;
    for (const auto& node : m_root->get_children()) {
        if (node->valid()) {
            const auto visits = node->get_visits();
            const auto eval = (visits > 0) ? node->get_eval(color) : 0.0f;
            if (visits > n_best_visits) {
                n_best_visits = visits;
                n_best = idx;
            }
            if (visits > 0 && eval > q_best_eval) {
                q_best_eval = eval;
                q_best = idx;
            }
        }
    ...
    // PG: n_best != q_best means unstable.
    // Note: PG mean_action is child-perspective value in [-1,1];
    // LZ get_eval(color) is `color`-perspective winrate in [0,1].
    // Both measure "good for current side", argmax is equivalent.
    return (n_best != q_best && n_best >= 0 && q_best >= 0);
}

// PhoenixGo: CheckBehind (see PhoenixGo mcts_engine.cc:646-668)
// True if best move's winrate < act_threshold (mapped to LZ [0,1] scale).
bool UCTSearch::check_behind() {
    if (!cfg_behind_overtime_enable) return false;
    const auto color = m_rootstate.board.get_to_move();
    auto best_visits = 0;
    auto best_eval = 0.0f;
    auto has_best = false;
    // PG get_best_move_mode=0: pick by visit count.
    for (const auto& node : m_root->get_children()) {
        if (!node->valid()) continue;
        const auto visits = node->get_visits();
        if (!has_best || visits > best_visits) {
            best_visits = visits;
            best_eval = (visits > 0) ? node->get_eval(color) : 0.0f;
            has_best = true;
        }
    }
    if (!has_best || best_visits == 0) return false;
    // PG: mean_action < act_threshold (0.0) means behind.
    // LZ mapping: winrate = (mean_action + 1) / 2, so threshold maps the same way.
    const auto winrate_threshold =
        (cfg_behind_overtime_act_threshold + 1.0f) / 2.0f;
    if (best_eval < winrate_threshold) {
        myprintf("PG behind overtime: best_eval=%.3f < threshold=%.3f\n",
                 best_eval, winrate_threshold);
        return true;
    }
    return false;
}

// PhoenixGo: GetSearchOvertimeUs (see PhoenixGo mcts_engine.cc:709-720)
// Returns extra time (centiseconds) when unstable or behind.
int UCTSearch::get_overtime_centis(const int base_time_centis) {
    if (base_time_centis > 0) {
        if (check_unstable()) {
            return static_cast<int>(base_time_centis
                                    * cfg_unstable_overtime_time_factor);
        }
        if (check_behind()) {
            return static_cast<int>(base_time_centis
                                    * cfg_behind_overtime_time_factor);
        }
    }
    return 0;
}
...
int UCTSearch::think(const int color, const passflag_t passflag) {
    ...
    // PhoenixGo time management state (see PhoenixGo mcts_engine.cc:722-733 Search)
    // total_time may be extended by overtime when base time expires.
    auto total_time = time_for_move;
    auto overtime_checked = false;
    auto last_early_stop_check = 0;
    ...
    do {
        ...
        // PhoenixGo: when base time expires, check for overtime
        // (see PhoenixGo mcts_engine.cc:729-730 GetSearchOvertimeUs)
        if (!overtime_checked && elapsed_centis >= time_for_move) {
            const auto overtime = get_overtime_centis(time_for_move);
            total_time = time_for_move + overtime;
            overtime_checked = true;
            if (overtime > 0) {
                myprintf("PhoenixGo overtime: +%d centis\n", overtime);
            }
        }
        ...
        keeprunning &= !stop_thinking(elapsed_centis, total_time);
        keeprunning &= have_alternate_moves(elapsed_centis, total_time);
    } while (keeprunning);
