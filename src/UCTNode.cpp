...
bool UCTNode::create_children(Network& network, std::atomic<int>& nodecount,
    ...
    // PhoenixGo: Always add PASS as a child (see PhoenixGo mcts_engine.cc:529)
    // PG: moves[ch_len++] = GoComm::COORD_PASS
    // LZ original: conditionally add PASS based on cfg_dumbpass/stm_eval.
    // To match PG playing style, always add PASS and apply dumb_pass
    // suppression below.
    auto pass_policy = raw_netlist.policy_pass;

    // PhoenixGo: dumb_pass policy suppression (see PhoenixGo mcts_engine.cc:312-320)
    ...
    if (nodelist.size() > PG_MAX_CHILDREN_PER_NODE) {
        std::partial_sort(begin(nodelist),
                          begin(nodelist) + PG_MAX_CHILDREN_PER_NODE,
                          end(nodelist),
...
UCTNode* UCTNode::uct_select_child(const int color, const bool is_root) {
    ...
    // PhoenixGo: virtual_loss_mode=0 (default) -> sigma_visit_count includes
    // virtual_loss. See PhoenixGo mcts_engine.cc:469-474 SelectChild.
    ...
    // PhoenixGo: sigma includes virtual_loss (virtual_loss_mode & 1 == 0)
    parentvisits += total_virtual_loss;

    // PhoenixGo: PUCT formula uses sqrt(max(sigma, 1.0))
    // See PhoenixGo mcts_engine.cc:473-474 SelectChild
    const auto numerator = std::sqrt(std::max(double(parentvisits), 1.0));
    // PhoenixGo: FPU uses default_act=0 (no reduction)
    // cfg_fpu_reduction defaults to 0.0 to match PhoenixGo's default_act
    ...
    for (auto& child : m_children) {
        ...
        // PhoenixGo: virtual_loss_mode=0 -> denom includes virtual_loss
        // See PhoenixGo mcts_engine.cc:496-497 SelectChild
