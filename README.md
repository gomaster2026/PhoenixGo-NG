# leela-zero-next — PhoenixGo 复活计划

本 fork 的目标是复活 PhoenixGo 权重（基于 AlphaGo Zero 论文），
通过 **Kaggle 免费 T4 GPU** + **社区算力贡献** 进行分布式训练。

**无需服务器、无需付费**，你只需要：
- 有 GPU 的电脑 → 运行 `node_client.py` 自对弈贡献棋谱
- 或贡献 Kaggle Notebook 的 T4 GPU 时间

详见 [`distributed/`](distributed/) 目录和 [`CONTRIBUTING_GUIDE.md`](CONTRIBUTING_GUIDE.md)。
